...
class EqualizedConv2d(chainer.Chain):
    def __init__(self, in_ch, out_ch, ksize, stride, pad):
        ...
        with self.init_scope():
            self.c = L.Convolution2D(
                in_ch, out_ch, ksize, stride, pad, initialW=w
            )
...
def minibatch_std(x):
    ...
    v = F.mean((x - F.broadcast_to(m, x.shape)) *
               (x - F.broadcast_to(m, x.shape)), axis=0, keepdims=True)
...
class GeneratorBlock(chainer.Chain):
    ...
    def __call__(self, x):
        h = F.unpooling_2d(x, 2, 2, 0, outsize=(
...
class Generator(chainer.Chain):
    def __init__(self, n_hidden=512, ch=512, max_stage=10, size=128):
        ...
        with self.init_scope():
            ...
            self.b4 = GeneratorBlock(ch // 2, ch // 2)
            self.out4 = EqualizedConv2d(ch // 2, 3, 1, 1, 0)
            self.b5 = GeneratorBlock(ch // 2, ch // 4)
            self.out5 = EqualizedConv2d(ch // 4, 3, 1, 1, 0)

    def make_hidden(self, batchsize):
        xp = self.xp
        z = xp.random.normal(size=(batchsize, self.n_hidden, 1, 1)) \
            .astype(np.float32)
        z /= xp.sqrt(xp.sum(z * z, axis=1, keepdims=True) /
                     self.n_hidden + 1e-8)
        return z
    ...
    def __call__(self, z, stage):
        ...
        if int(stage) % 2 == 0:
            ...
            x_0 = out_prev(F.unpooling_2d(
                h, 2, 2, 0, outsize=(2 * h.shape[2], 2 * h.shape[3])))
        ...
        if chainer.configuration.config.train:
            ...
            scale = int(self.size // x.data.shape[2])
            return F.unpooling_2d(
                x, scale, scale, 0, outsize=(self.size, self.size)
            )
...
class Discriminator(chainer.Chain):
    def __init__(self, ch=512, max_stage=10, pooling_comp=1.0):
        super(Discriminator, self).__init__()
        self.max_stage = max_stage
        self.pooling_comp = pooling_comp  # compensation of ave_pool is 0.5-Lipshitz
        with self.init_scope():
            self.in5 = EqualizedConv2d(3, ch // 4, 1, 1, 0)
            self.b5 = DiscriminatorBlock(ch // 4, ch // 2, pooling_comp)
            self.in4 = EqualizedConv2d(3, ch // 2, 1, 1, 0)
            self.b4 = DiscriminatorBlock(ch // 2, ch // 2, pooling_comp)
    ...
    def __call__(self, x, stage):
        ...
        if int(stage) % 2 == 0:
            ...
            h0 = F.leaky_relu(
                fromRGB0(self.pooling_comp *
                         F.average_pooling_2d(x, 2, 2, 0))
            )
